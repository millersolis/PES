...
static uint8_t ranging_rx_retry_counter = 0;
...
bool retry_ranging_rx() {
#ifdef INFINITE_RETRIES_RX
	return true;
#endif

	if (ranging_rx_retry_counter < MAX_RNG_RETRY) {
		++ranging_rx_retry_counter;
		return true;
	}

	ranging_rx_retry_counter= 0;
	return false;
}
...
rid_state_t perform_ranging()
{
    ...
    if (rangingInitStatus != STATUS_RECEIVE_OK) {
		if (retry_ranging_rx()) {
			return STATE_RANGING;
		}
		return STATE_DISCOVERY;
